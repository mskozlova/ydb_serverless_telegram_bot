...
def handle_register(message: Message, bot: TeleBot):
    ...
    bot.set_state(message.from_user.id, states.RegisterState.first_name, message.chat.id)
...
def handle_show_data(message: Message, bot: TeleBot):
    ...
    if not current_data:
        bot.send_message(message.chat.id, texts.not_registered_message, reply_markup=keyboards.empty)
        return
...
def handle_delete_account(message: Message, bot: TeleBot):
    ...
    bot.set_state(message.from_user.id, states.DeleteAccountState.are_you_sure, message.chat.id)
